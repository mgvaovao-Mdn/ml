...
def _subtotal(ws, row, label, amount, ncols, bg=BLUE_PALE, pct=None, total_ref=220167):
    ws.merge_cells(start_row=row, start_column=1,
...
for r, label, amt, note, bg in [
    (9,  "Personnel — direction + équipe technique (12 postes)", 152436, "69.2 % des coûts directs", BLUE_LIGHT),
    (10, "Équipement et logiciels",                               8000,  " 3.6 % des coûts directs", OFFWHITE),
    (11, "Collecte et traitement des données (20 collecteurs + 9 annotateurs)", 14231, " 6.5 % des coûts directs", BLUE_LIGHT),
    (12, "Déplacements et terrain (tournée 14 régions)",         20000,  " 9.1 % des coûts directs", OFFWHITE),
    (13, "Formation et ateliers (tournée nationale 18 villes)",  22000,  "10.0 % des coûts directs", BLUE_LIGHT),
    (14, "Communication et mobilisation",                         2500,  " 1.1 % des coûts directs", OFFWHITE),
...
for r, label, amt, bg, txt_color, cap_lbl in [
    (18, "Frais généraux / indirects (5 %)",  11008,  BLUE_PALE,  BLUE_DARK, "5 % × 220 167"),
    (19, "BUDGET CASH TOTAL DEMANDÉ",        231175,  GREEN_DARK, WHITE,     "< 250 000 USD ✓"),
...
personnel = [
    ("Fondateur et Directeur général — conseil stratégique (50 %)",
     "1 250 × 18 mois", "mois", 18, 1250, 22500),
    ("Directrice locale (100 %)",
     "1 333 × 18 mois  ≈ 6 000 000 MGA/mois", "mois", 18, 1333, 23994),
    ("Chef de projet technique ML/Dev (100 %)",
     "1 333 × 18 mois  ≈ 6 000 000 MGA/mois", "mois", 18, 1333, 23994),
    ("Chef de projet Data et Opérations (100 %)",
     "1 333 × 18 mois  ≈ 6 000 000 MGA/mois", "mois", 18, 1333, 23994),
    ("Chef de projet Partenariats et Impact (recrutement M7, 100 %)",
     "1 333 × 12 mois (M7–M18)  ≈ 6 000 000 MGA/mois", "mois", 12, 1333, 15996),
    # Équipe technique — 7 développeurs à 1 500 000 MGA (333 USD) / mois
    ("Développeur backend (100 %)",
     "333 × 18 mois  ≈ 1 500 000 MGA/mois", "mois", 18, 333, 5994),
    ("Développeur frontend (100 %)",
     "333 × 18 mois  ≈ 1 500 000 MGA/mois", "mois", 18, 333, 5994),
    ("Développeur mobile (100 %)",
     "333 × 18 mois  ≈ 1 500 000 MGA/mois", "mois", 18, 333, 5994),
    ("Développeur IA / ML engineer (100 %)",
     "333 × 18 mois  ≈ 1 500 000 MGA/mois", "mois", 18, 333, 5994),
    ("Data scientist (100 %)",
     "333 × 18 mois  ≈ 1 500 000 MGA/mois", "mois", 18, 333, 5994),
    ("DevOps engineer (100 %)",
...
row += 1
_subtotal(ws, row, "Sous-total Personnel (direction + équipe technique)", 152436, NCOLS)
...
row += 1
_section_hdr(ws, row, "3. COLLECTE ET TRAITEMENT DES DONNÉES", NCOLS)

collecte_annotation = [
    ("20 collecteurs terrain — 1 à 2 par dialecte × 18 dialectes (recrutés dans les communautés, "
     "priorité aux femmes, rémunération équitable)",
     "133 USD (600 000 MGA) × 20 pers. × 4 mois actifs", "pers.×mois", 80, 133, 10640),
    ("9 annotateurs-transcripteurs — vérification des transcriptions, "
     "alignement phonème-graphème, contrôle qualité",
     "133 USD (600 000 MGA) × 9 pers. × 3 mois actifs", "pers.×mois", 27, 133, 3591),
]

for i, (desc, calc, unite, qty, uc, total) in enumerate(collecte_annotation):
    row += 1
    bg = ALT[i % 2]
    _data_row(ws, row, desc, calc, unite, qty, uc, total, bg)

row += 1
_subtotal(ws, row, "Sous-total Collecte et traitement des données", 14231, NCOLS)

# ── SECTION 4 : DÉPLACEMENTS ET TERRAIN ──────────────────────────────────
row += 1
_section_hdr(ws, row, "4. DÉPLACEMENTS ET TERRAIN", NCOLS)

travel = [
    ("Missions régionales — avions intérieurs et taxi-brousse (14 régions, 2 visites par région)",
    ...
    ("Transport local pour les coordinateurs de collecte (déplacements intracommunautaires)",
     "50 USD × 20 coordinateurs × 4 mois actifs", "pers.×mois", 80, 50, 4000),
    ("Location de salles communautaires pour les sessions de collecte et réserve imprévus",
     "21 USD × 28 sessions + réserve opérationnelle", "session", 28, 21, 600),
]
...
row += 1
_subtotal(ws, row, "Sous-total Déplacements et terrain", 20000, NCOLS)

# ── SECTION 5 : FORMATION ET ATELIERS ────────────────────────────────────
row += 1
_section_hdr(ws, row, "5. FORMATION ET ATELIERS", NCOLS)

workshops = [
    ("Formation des coordinateurs locaux — protocoles, outils d'enregistrement (Antananarivo)",
     "Salle + matériel + indemnités — 2 jours", "forfait", 1, 1500, 1500),
    ("Tournée nationale de formation — 18 villes : transport intercity (avion et taxi-brousse)",
     "300 USD × 18 étapes", "étape", 18, 300, 5400),
    ("Tournée nationale — hébergement équipe (18 villes × 3 nuits en moyenne)",
     "50 USD/nuit × 18 villes × 3 nuits", "nuit", 54, 50, 2700),
    ("Tournée nationale — location de salles de conférence (18 villes)",
     "200 USD × 18 salles", "salle", 18, 200, 3600),
    ("Tournée nationale — matériel et supports pédagogiques (18 ateliers)",
     "128 USD × 18 ateliers", "atelier", 18, 128, 2300),
    ("Atelier de restitution final — communauté NLP africaine (Antananarivo)",
     "Organisation, accueil participants, matériel de présentation", "forfait", 1, 3500, 3500),
    ("Participation à une conférence NLP régionale (IndabaX ou AfricaNLP)",
     "Inscription + transport + hébergement", "forfait", 1, 3000, 3000),
]
...
row += 1
_subtotal(ws, row, "Sous-total Formation et ateliers", 22000, NCOLS)

# ── SECTION 6 : COMMUNICATION ET MOBILISATION ────────────────────────────
row += 1
_section_hdr(ws, row, "6. COMMUNICATION ET MOBILISATION", NCOLS)

comms = [
    ("Rapports d'avancement semestriels (mise en page, traduction FR/EN)",
     "100 × 6 rapports (M3, M6, M9, M12, M15, M18)", "rapport", 6, 100, 600),
    ("Outils numériques : gestion projet, hébergement web, licences",
     "50/mois × 18 mois", "mois", 18, 50, 900),
    ("Stratégie de communication et réseaux sociaux (identité visuelle, contenus numériques)",
     "Forfait production contenus + community management", "forfait", 1, 1000, 1000),
]
...
row += 1
_subtotal(ws, row, "Sous-total Communication et mobilisation", 2500, NCOLS)
...
c2 = ws.cell(row=row, column=NCOLS - 1, value=220167)
...
c1.alignment = Alignment(horizontal="right", vertical="center")
c1.border    = thin_border
c2 = ws.cell(row=row, column=NCOLS - 1, value=11008)
c2.font          = Font(name="Calibri", bold=True, size=10, color=BLUE_DARK)
...
c1.alignment = Alignment(horizontal="right", vertical="center")
c1.border    = thick_border
c2 = ws.cell(row=row, column=NCOLS - 1, value=231175)
...
summary = [
    ("1. Personnel — direction + équipe technique",  152436, "69.2 %",
     "12 postes : 5 direction + 7 développeurs à 1 500 000 MGA/mois — 18 mois"),
    ("2. Équipement et logiciels",                     8000, " 3.6 %",
     "Laptops, matériel d'enregistrement terrain, connectivité"),
    ("3. Collecte et traitement des données",          14231, " 6.5 %",
     "20 collecteurs × 133 USD × 4 mois + 9 annotateurs × 133 USD × 3 mois"),
    ("4. Déplacements et terrain",                    20000, " 9.1 %",
     "14 régions × 2 missions + transport coordinateurs + salles"),
    ("5. Formation et ateliers",                      22000, "10.0 %",
     "Tournée nationale 18 villes + coordinateurs + restitution + IndabaX"),
    ("6. Communication et mobilisation",               2500, " 1.1 %",
     "Rapports semestriels, outils numériques, stratégie réseaux sociaux"),
